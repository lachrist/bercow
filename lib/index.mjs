...
const { parse: parseList } = ListLang;
const { max } = Math;
const { isArray } = Array;
...
const parseListEither = wrap(parseList);
    ...
    return fromEither(
      await bindRightAsync(await statEitherAsync(main), async (stat) => {
        ...
        if (stat.isDirectory()) {
          ...
          either = await readFileEitherAsync(join(main, ordering), "utf8");
          ...
          either = bindRight(either, parseListEither);
          ...
          either = await bindRightAsync(
            ...
            async ({ relative_test, test, hash }) => {
              ...
              return memo === digest
                ? createRight(createMemoizedResult(digest))
                : mapRight(
                    await spawnEitherAsync(command, argv, {
                      timeout,
                      cwd,
                      stdio,
                      env: {
                        ...env,
                        [main_var]: relative_main,
                        [test_var]: relative_test,
                      },
                    }),
                    (unit) => createSuccessResult(digest),
                  );
            },
          );
        }
