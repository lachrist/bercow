...
export const bercow = async (options, home) => {
  ...
  try {
    await visitDirectoryAsync(resolvePath(options["target-directory"], home), context);
  } finally {
...
const lintAsync = async (path, context) => {
  ...
  if (context.achievements.has(hash)) {
    ...
    const new_content = await context.lint({ path, content }, context.done.slice());
    const new_buffer = toBuffer(content, context.encoding);
