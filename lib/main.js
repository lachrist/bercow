...
const loop = (path, memo, options) => {
  if (path === "" || path.endsWith("/")) {
    assert(
      memo === null,
      Error,
      "directory entries should not an associated value (ie the memoization)",
    );
    ...
    if (options.memo && digest === memo) {
      ...
      try {
        ...
        if (error instanceof ExitSpawnError) {
          ...
          } /* c8 ignore start */ /* eslint-disable no-empty */ catch (error) {} /* eslint-enable no-empty */ /* c8 ignore stop */
        }
        throw error;
      }
      options.after(main, test);
      memo = digest;
    }
  }
  return memo;
