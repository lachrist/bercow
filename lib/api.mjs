...
import {isNotEmptyString} from "./util.mjs";
import {makeCache, updateCache, resetCache, readCache, openCache, closeCache} from "./cache.mjs";
...
const lintAsync = async (path, context) => {
  ...
  if (!context.achievements.has(hashFile(file))) {
    saveFile(file, await context.lint(cleanupFile(file)));
    if (!context.achievements.has(hashFile(file))) {
