...
const splitProgramBody = (statements) => {
  ...
  while (!step.done) {
    if (isImport(step.value)) {
      throw new Error("Import statement should appear before any other statements");
    }
...
const compareImportStatement = (statement1, statement2, base, ordering) => {
  const {source:{value:source1}} = statement1;
  const {source:{value:source2}} = statement2;
  const ranking1 = getSourceRanking(source1);
  const ranking2 = getSourceRanking(source2);
  const comparison = ranking1 - ranking2;
  if (ranking1 !== ranking2) {
    return ranking1 - ranking2;
  } else if (ranking1 !== INTERNAL_SOURCE_RANKING) {
    return 0;
  } else {
    return getSourceOrder(source1, base, ordering) - getSourceOrder(source2, base, ordering);
  }
};

const isNodeExternalSource = (source) => source.startsWith("node:");

const isInternalSource = (source) => source[0] === "/" || source[0] === ".";

const isExternalSource = (source) => !isInternalSource(source);
...
export const lint = async (config) => {
  ...
  return async (file, ordering) => {
    ...
    if (config.fix) {
      ...
      sorted_head.sort((statement1, statement2) => compareImportStatement(statement1, statement2, ordering));
      ...
      while (!step2.done) {
        if (compareImportStatement(step1.value, step2.value, ordering) > 0) {
          const {value:{loc:{line:line1, column:column1}, source:{value:source1}}} = step1;
          const {value:{loc:{line:line2, column:column2}, source:{value:source2}}} = step2;
          throw new Error(
            `Ordering mismatch between ${JSON.stringify(source1)} at ${line1}:${column1} and ${JSON.stringify(source1)} at ${line2}:${column2}`
          );
        }
        step1 = step2;
        step2 = iterator.next();
